...
class IFRSForensicEngine:
    ...
    def assess_data_quality(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        score = 0.0
        if has_s1 and has_s2: score += 0.4  # Source Integrity
        if has_output: score += 0.3          # Completeness
        if len(data.get("governance", {})) > 0: score += 0.3 # Traceability
...
# =====================================================================
# 3. PIPELINE RUNNER & GIT BATCH EXECUTOR
# =====================================================================
...
def run_pipeline(input_dir: str = "data/raw_html", output_dir: str = "data/processed"):
    ...
    # Process all HTML corporate reports in the input directory
    ...
    print(f"[+] Diagnostic Report saved to: {json_path}")
    print(f"[+] CSV Audit Log saved to: {csv_path}")
